...
import xqa.qpid.jms.MessageBroker;
import xqa.qpid.jms.MessageMaker;
...
public class XQueryResource {
    ...
    TemporaryQueue shardReplyToQueue;

    public XQueryResource(MessageBrokerConfiguration messageBrokerConfiguration, String serviceId) throws Exception {
        synchronized (this) {
            this.serviceId = serviceId;

            messageBroker = new MessageBroker(
                    messageBrokerConfiguration.getHost(),
                    messageBrokerConfiguration.getPort(),
                    messageBrokerConfiguration.getUserName(),
                    messageBrokerConfiguration.getPassword(),
    ...
    private synchronized void sendAuditEvent(QueryBalancerEvent.State eventState, String correlationId, String xquery) throws Exception {
        QueryBalancerEvent queryBalancerEvent = new QueryBalancerEvent(
                serviceId,
                correlationId,
    ...
    private synchronized void sendXQueryToShards(@NotNull @Valid XQueryRequest xquery, String correlationId) throws Exception {
